...
COPY run.sh /
COPY libressl.patch /
#RUN buildDeps='git build-base mysql-dev alpine-sdk openssl-dev libwebsockets-dev c-ares-dev util-linux-dev hiredis-dev curl-dev libxslt docbook-xsl'; \
RUN buildDeps='git build-base mysql-dev libressl-dev libwebsockets-dev c-ares-dev util-linux-dev hiredis-dev curl-dev libxslt docbook-xsl'; \
    chmod +x /run.sh && \
    mkdir -p /var/lib/mosquitto && \
    touch /var/lib/mosquitto/.keep && \
    mkdir -p /etc/mosquitto.d && \
    apk update && \
    apk add $buildDeps hiredis mariadb-client-libs libwebsockets libuuid c-ares libressl curl ca-certificates && \
